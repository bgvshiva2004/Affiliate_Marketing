...
export const ShoppingSpot = ({ initialOpen = false }: ShoppingSpotProps) => {
  const controls = useAnimation()
  const [isOpen, setIsOpen] = useState(initialOpen)

  // Helper functions for local storage
  const getIsOpen = () => localStorage.getItem('shoppingSpotOpen') === 'true'

  const updateStateFromStorage = () => {
    const state = getIsOpen()
    setIsOpen(state)
    controls.start({ height: state ? '80vh' : '0px' })
  }
  ...
  }, [localStorage.getItem('shoppingSpotOpen')])

  const handleDragEnd = (event: any, info: any) => {
    if (info.offset.y < -50) {
      localStorage.setItem('shoppingSpotOpen', 'true')
      updateStateFromStorage()
    } else if (info.offset.y > 50) {
      localStorage.setItem('shoppingSpotOpen', 'false')
      updateStateFromStorage()
    }
  }
  ...
  const toggleDrawer = () => {
    ...
    localStorage.setItem('shoppingSpotOpen', newState.toString())
    updateStateFromStorage()
  }
